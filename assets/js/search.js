...
function highlightKeywords(content, previewStartPosition, matchMetadata) {
  ...
  for (var keyword in matchMetadata) {
    ...
    for (
      ...
      if (
        hitPosition >= previewStartPosition &&
        hitPosition < previewStartPosition + PREVIEW_SIZE
      ) {
  ...
  if (Object.keys(matchMap).length !== 0) {
    ...
    for (var wordPosition in matchMap) {
      var wordEnd =
        parseInt(wordPosition) + parseInt(matchMap[wordPosition]) + 1;
      processedPreview +=
        content.substring(currPosition, wordPosition) +
        "<b>" +
        content.substring(wordPosition, wordEnd) +
        "</b>";
      currPosition = wordEnd;
    }

    if (wordEnd < previewStartPosition + PREVIEW_SIZE) {
      processedPreview += content.substring(
        currPosition,
        previewStartPosition + PREVIEW_SIZE
      );
    }
...
function returnStartOfPreview(content, firstPosition) {
  if (firstPosition - NUM_LEADING_CHARS <= 0) {
    ...
    for (
      var index = firstPosition - NUM_LEADING_CHARS;
      index < firstPosition;
      index++
    ) {
...
function returnResultsList(results) {
  ...
  for (var i = 0; i < results.length; i++) {
    ...
    if (!resultObject || !resultObject.hasOwnProperty("title")) {
      continue;
    }
    var resultTitle = resultObject["title"].substring(0, titleTruncateLength);

    if (resultObject["title"].length > titleTruncateLength) {
      var indexOfLastWord = resultObject["title"]
        .substring(0, titleTruncateLength)
        .lastIndexOf(" ");
    ...
    var previewStartPosition = returnStartOfPreview(
      resultObject["content"],
      firstPosition
    );

    // Process the preview to embolden keywords
    var processedPreview = highlightKeywords(
      resultObject["content"],
      previewStartPosition,
      matchMetadata
    );
    ...
    searchPara +=
      '<p class="search-content permalink">' + resultObject["url"] + "</p><br>";
...
function paginateSearchResults() {
  ...
  searchPageIndicator.innerHTML =
    "Page " + (currentPageIndex + 1) + " of " + pageResults.length;

  var searchResults = document.getElementById("search-results");
  searchResults.innerHTML = returnResultsList(pageResults[currentPageIndex]);
  document.getElementsByClassName("search-results-display")[0].style.display =
    "block";
}
